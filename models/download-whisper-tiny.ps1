# Download openai/whisper-tiny as a Hugging Face Whisper safetensors folder (smaller test/demo).
# Speech-to-text (audio -> text). Not a chat model. Not CTranslate2 model.bin.
# Source: https://huggingface.co/openai/whisper-tiny
# Usage (PowerShell):  .\models\download-whisper-tiny.ps1
# Or double-click / run: models\download-whisper-tiny.cmd

$ErrorActionPreference = 'Stop'

$ModelsRoot = if ($PSScriptRoot) { $PSScriptRoot } else { Split-Path -Parent $MyInvocation.MyCommand.Path }
$Dest = Join-Path $ModelsRoot 'whisper-tiny'
$Base = 'https://huggingface.co/openai/whisper-tiny/resolve/main'
...
function Download-HfFile([string]$Name) {
    ...
    try {
        ...
        Write-Host 'https://huggingface.co/openai/whisper-tiny' -ForegroundColor Red
        throw
    }
}
...
Write-Host 'Downloading model.safetensors (~150MB) ...'
