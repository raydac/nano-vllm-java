...
download() {
  ...
  if ! curl_resume "$f" "$BASE/$f" "${AUTH[@]}"; then
    ...
    echo "Model card: https://huggingface.co/openai/whisper-tiny" >&2
    exit 1
  fi
}
...
echo "Downloading model.safetensors (~150MB) ..."
...
echo "Load: LlmModelFactory.make(Path.of(\"models/whisper-tiny\")); model.transcribe(wavPath)"
