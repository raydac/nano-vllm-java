...
# Usage (PowerShell):  .\models\download-piper-en-lessac-medium.ps1

$ErrorActionPreference = 'Stop'

$ModelsRoot = if ($PSScriptRoot) { $PSScriptRoot } else { Split-Path -Parent $MyInvocation.MyCommand.Path }
$Dest = Join-Path $ModelsRoot 'piper-en-lessac-medium'
...
Write-Host 'Downloading Piper Lessac medium ONNX + sidecar ...'
Download-HfFile 'en_US-lessac-medium.onnx'
Download-HfFile 'en_US-lessac-medium.onnx.json'
...
Write-Host 'Try: synthesize "Hello world" (Example TTS session, or SynthesizeHelloWorld)'
