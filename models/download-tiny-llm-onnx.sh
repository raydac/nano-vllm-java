...
curl_resume config.json "$ONNX_BASE/config.json"
curl_resume generation_config.json "$ONNX_BASE/generation_config.json"
...
for f in tokenizer.json tokenizer_config.json special_tokens_map.json tokenizer.model; do
  curl_resume "$f" "$TOK_BASE/$f" || true
done

echo "Downloading onnx/model.onnx (fp32) ..."
curl_resume onnx/model.onnx "$ONNX_BASE/onnx/model.onnx"
